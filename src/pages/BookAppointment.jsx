...
//   // ── Fetch 15-min micro slots whenever date + slotTime change ─────────────
//   useEffect(() => {
//     if (!form.preferredDate || !form.slotTime) {
//       setMicroSlots([]); setAllSlotsFull(false); return;
//     }
//     setMicroLoading(true);
//     citizenAxios
//       .get("/availability/micro-slots", {
//         params: { date: form.preferredDate, slotTime: form.slotTime }
//       })
//       .then(r => {
//         setMicroSlots(r.data.data.slots || []);
//         setAllSlotsFull(r.data.data.allFull || false);
//       })
//       .catch(() => { setMicroSlots([]); setAllSlotsFull(false); })
...
//   const nextDisabled = (() => {
//     if (step===0) return !form.fullName || !form.mobileNumber || !form.address;
//     if (step===1) return !form.preferredDate || !form.slotTime || !form.microSlot; // ← requires microSlot
//     if (step===2) return !form.purpose || !form.ward;
//     if (step===3) return !form.visitorPhoto;
//     return false;
//   })();
...
//       fd.append("preferredDate",    form.preferredDate);
//       fd.append("slotTime",         form.slotTime);
//       fd.append("microSlot",        form.microSlot);       // ← NEW
//       fd.append("purpose",          form.purpose);
//       fd.append("numberOfVisitors", form.numberOfVisitors);
//       fd.append("visitedBefore",    String(form.visitedBefore));
//       fd.append("ward",             form.ward);
...
//                 ["Date",       formatShortDate(booked.preferredDate)],
//                 ["Slot",       booked.slotTime],
//                 ["Your Time",  booked.microSlot || "—"],   // ← show assigned 15-min slot
//                 ["Status",     "⏳ Pending — Admin approval बाकी आहे"],
//               ].map(([k,v]) => (
//                 <div key={k} style={{ display:"flex", justifyContent:"space-between", padding:"9px 0", borderBottom:"1px solid #d4eff5", fontSize:13 }}>
//                   <span style={{ color:"#5a7a88", fontWeight:600 }}>{k}</span>
...
//         /* ── Micro slot grid ── */
//         .micro-slot-grid { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 4px; }
//         .micro-btn {
//           padding: 8px 14px;
//           border-radius: 8px;
//           font-size: 12.5px;
...
//           color: var(--text-bd);
//         }
//         .micro-btn:hover:not(:disabled) { border-color: var(--teal); background: #edf9f0; color: var(--teal-dk); transform: translateY(-1px); }
//         .micro-btn.sel   { border: 2px solid var(--teal); background: #dff2f6; color: var(--teal-dk); box-shadow: 0 2px 8px rgba(58,154,175,0.2); }
//         .micro-btn.taken { border-color: #f5c6cb; background: #f9f9f9; color: #9ca3af; cursor: not-allowed; text-decoration: line-through; }
...
//         .review-card-title {
//           font-size: 11.5px; font-weight: 800; color: var(--teal-dk);
...
//           border: 1.5px solid #c0a850; background: #fffef0;
//           color: var(--text-sm); font-weight: 700; font-size: 13.5px;
//           cursor: pointer; font-family: 'Noto Sans','Mukta','Plus Jakarta Sans',sans-serif;
//           transition: all .18s;
//         }
//         .nav-cancel:hover { background:#f5edd0; border-color:var(--gold); color:var(--gold2); }
//         .nav-continue {
//           padding: 11px 34px; border-radius: 8px; border: none;
//           background: linear-gradient(135deg,#3a9aaf,#2e8a9e); color: #fff;
...
//           font-family:'Noto Sans','Mukta','Plus Jakarta Sans',sans-serif;
//           border:none; box-shadow:0 3px 10px rgba(58,154,175,0.3); transition:all .18s;
//         }
//         .photo-btn-primary:hover { transform:translateY(-1px); box-shadow:0 5px 14px rgba(58,154,175,0.4); }
//         .photo-btn-secondary {
//           padding:11px 24px; border-radius:8px;
//           border:1.5px solid var(--teal); background:#fffef8; color:var(--teal-dk);
//           font-weight:700; font-size:13.5px; cursor:pointer;
//           font-family:'Noto Sans','Mukta','Plus Jakarta Sans',sans-serif; transition:all .18s;
//         }
//         .photo-btn-secondary:hover { background:#dff2f6; }
...
//           font-size:14px; font-weight:700; color:var(--text-bd);
//           padding:8px 20px; border-radius:8px;
//           border:1.5px solid var(--border); background:#fffef8; transition:all .15s;
//         }
//         .radio-label.selected { border-color:var(--teal); background:#dff2f6; color:var(--teal-dk); }
//         .radio-label input { accentColor:var(--teal); width:15px; height:15px; }
//       `}</style>
...
//                               onClick={() => setForm(p => ({
//                                 ...p,
//                                 preferredDate: av.date,
//                                 slotTime: "", slotStart: "", slotEnd: "",
//                                 microSlot: "",   // ← reset micro slot on date change
//                               }))}>
//                               📅 {formatShortDate(av.date)}
//                               <div className="sub">{av.timeSlots?.length} slot{av.timeSlots?.length!==1?"s":""} available</div>
//                             </button>
...
//                                   onClick={() => setForm(p => ({
//                                     ...p,
//                                     slotTime: str, slotStart: slot.start, slotEnd: slot.end,
//                                     microSlot: "",   // ← reset micro slot on parent slot change
//                                   }))}>
//                                   ⏰ {str}
//                                 </button>
//                               );
...
//                       {/* ── 15-minute micro slot selection ── */}
//                       {form.preferredDate && form.slotTime && (
//                         <div className="field">
//                           <label>
//                             15-Minute Appointment Slot निवडा <span className="req">*</span>
//                           </label>
...
//                           ) : (
//                             <>
//                               <div className="micro-slot-grid">
//                                 {microSlots.map(slot => (
//                                   <button
//                                     key={slot.microSlot}
//                                     type="button"
//                                     disabled={slot.booked}
//                                     className={`micro-btn${slot.booked?" taken":form.microSlot===slot.microSlot?" sel":""}`}
//                                     onClick={() => !slot.booked && setForm(p => ({ ...p, microSlot: slot.microSlot }))}
//                                   >
//                                     {slot.booked ? "🔴" : form.microSlot === slot.microSlot ? "✅" : "🟢"} {slot.microSlot}
//                                   </button>
//                                 ))}
//                               </div>
...
//                           ["Slot",      form.slotTime],
//                           ["My Time",   form.microSlot],   // ← NEW row
//                         ]
//                       },
...
//                                 value={form.microSlot}
//                                 onChange={e => setForm(p => ({ ...p, microSlot: e.target.value }))}
//                               >
//                                 <option value="">-- 15-minute slot निवडा --</option>
//                                 {microSlots.map(slot => (
//                                   <option
//                                     key={slot.microSlot}
//                                     value={slot.microSlot}
//                                     disabled={slot.booked}
//                                   >
...
//   // ── Generate + fetch 15-min micro slots whenever date + slotTime change ────
//   useEffect(() => {
//     if (!form.preferredDate || !form.slotTime) {
//       setMicroSlots([]); setAllSlotsFull(false); return;
//     }

//     // 1️⃣ Immediately build frontend-generated slots so dropdown is never empty
...
//     // 2️⃣ Fetch booked status from API and merge
//     setMicroLoading(true);
//     citizenAxios
//       .get("/availability/micro-slots", {
//         params: { date: form.preferredDate, slotTime: form.slotTime }
//       })
//       .then(r => {
//         const apiSlots = r.data?.data?.slots || [];
//         // Merge: mark booked slots returned by API
//         const bookedSet = new Set(apiSlots.filter(s => s.booked).map(s => s.microSlot));
//         const merged = generated.map(s => ({
//           ...s,
//           booked: bookedSet.has(s.microSlot),
//         }));
//         setMicroSlots(merged);
//         setAllSlotsFull(merged.length > 0 && merged.every(s => s.booked));
//       })
//       .catch(() => {
//         // API failed — keep the generated slots as all-available
//         setMicroSlots(generated);
//         setAllSlotsFull(false);
//       })
//       .finally(() => setMicroLoading(false));
...
//   // ── Normalize slot string so "07:00-07:15" and "07:00 - 07:15" always match ──
//   const normalizeSlot = (s) => (s || "").replace(/\s*-\s*/g, "-").trim();

//   // ── Core function: fetch booked status and merge into generated slots ──────
//   const fetchAndMergeSlots = (date, slotTime, generated, currentMicroSlot) => {
//     return citizenAxios
//       .get("/availability/micro-slots", { params: { date, slotTime } })
//       .then(r => {
//         const apiSlots = r.data?.data?.slots || [];

//         // Build a Set of normalized booked slot strings
//         const bookedSet = new Set(
//           apiSlots.filter(s => s.booked).map(s => normalizeSlot(s.microSlot))
//         );
...
//         // If the currently selected slot just became booked → clear it immediately
...
//         return bookedSet; // return for use in pre-submit check
//       });
//   };
...
//     // 1️⃣ Immediately show frontend-generated slots — dropdown is never empty
//     const generated = generateMicroSlots(form.slotTime).map(s => ({ microSlot: s, booked: false }));
//     setMicroSlots(generated);
//     setAllSlotsFull(false);

//     // 2️⃣ Fetch real booked status from API and merge
//     setMicroLoading(true);
//     fetchAndMergeSlots(form.preferredDate, form.slotTime, generated, form.microSlot)
//       .catch(() => {
//         setMicroSlots(generated); // API failed — keep all slots available
//         setAllSlotsFull(false);
//       })
//       .finally(() => setMicroLoading(false));
//   }, [form.preferredDate, form.slotTime]);
...
//       // 🔒 Pre-submit: re-fetch booked slots to catch any last-second bookings
//       try {
//         const generated = generateMicroSlots(form.slotTime).map(s => ({ microSlot: s, booked: false }));
//         const r = await citizenAxios.get("/availability/micro-slots", {
//           params: { date: form.preferredDate, slotTime: form.slotTime }
//         });
//         const apiSlots = r.data?.data?.slots || [];
//         const bookedSet = new Set(apiSlots.filter(s => s.booked).map(s => normalizeSlot(s.microSlot)));

//         // Refresh the displayed slots so UI is always fresh
...
//           setForm(prev => ({ ...prev, microSlot: "" }));
//           setSubmitting(false);
//           return; // abort submit — send user back to pick another slot
//         }
//       } catch {
//         // If pre-check fails, proceed — backend will be the final guard
//       }
//       const fd = new FormData();
//       fd.append("citizenId",        citizen._id          || "");
//       fd.append("fullName",         form.fullName);
...
//   // ── Build bookedSet from API response — handles both success:true and success:false ──
//   const extractBookedSet = (responseData) => {
//     // Case 1: success:true  → { data: { slots: [...] } }
//     // Case 2: success:false → backend duplicate-booking error (slot IS booked)
//     if (responseData?.success === false) {
//       // Extract slot label from error message if present, else return empty set
//       // The actual slot list still needs to come from a fresh call — just return empty
//       return new Set();
//     }
//     const slots = responseData?.data?.slots || responseData?.slots || [];
//     return new Set(
//       slots.filter(s => s.booked).map(s => normalizeSlot(s.microSlot))
//     );
//   };
...
//       // If selected slot just became booked — clear it and warn
//       if (currentMicroSlot && bookedSet.has(normalizeSlot(currentMicroSlot))) {
//         showToast("⚠️ तुम्ही निवडलेला slot आत्ताच book झाला. कृपया दुसरा slot निवडा.", "error");
//         setForm(prev => ({ ...prev, microSlot: "" }));
//       }
...
//     // 1️⃣ Show all slots immediately (all available) — no empty dropdown
//     const generated = generateMicroSlots(form.slotTime).map(s => ({ microSlot: s, booked: false }));
//     setMicroSlots(generated);
//     setAllSlotsFull(false);

//     // 2️⃣ Overlay real booked status from API
//     setMicroLoading(true);
//     refreshMicroSlots(form.preferredDate, form.slotTime, generated, form.microSlot)
//       .finally(() => setMicroLoading(false));

//     // 3️⃣ Poll every 20 seconds so booked slots update in real-time
//     const poll = setInterval(() => {
//       // Re-read current form values via a fresh generated list
//       const gen = generateMicroSlots(form.slotTime).map(s => ({ microSlot: s, booked: false }));
//       setForm(prev => {
//         refreshMicroSlots(form.preferredDate, form.slotTime, gen, prev.microSlot);
//         return prev;
//       });
//     }, 20000);

//     return () => clearInterval(poll); // cleanup on unmount or slot change
//   }, [form.preferredDate, form.slotTime]);
...
//         // Refresh UI with latest booked status
//         const merged = generated.map(s => ({ ...s, booked: bookedSet.has(normalizeSlot(s.microSlot)) }));
//         setMicroSlots(merged);
...
//                                   // Double-check: block if slot is marked booked in current state
//                                   const slot = microSlots.find(s => s.microSlot === chosen);
//                                   if (slot?.booked) {
//                                     showToast("❌ हा slot आधीच book झाला आहे. कृपया दुसरा निवडा.", "error");
//                                     return;
//                                   }
...
//   const refreshMicroSlots = async (date, slotTime, generated, currentMicroSlot) => {
//     try {
//       // Fix: axios encodes spaces as "+" in params → "07:00+-+09:00" breaks backend
//       // Use manual query string so spaces become %20
//       const qs = `date=${encodeURIComponent(date)}&slotTime=${encodeURIComponent(slotTime)}`;
//       const r = await citizenAxios.get(`/availability/micro-slots?${qs}`);
//       const bookedSet = extractBookedSet(r.data);
...
//     // 3️⃣ Poll every 20 seconds — reads latest microSlot via closure over captured date/slotTime
...
//       // Get latest selected microSlot from DOM-independent state snapshot
//       setForm(prev => {
//         const latestMicro = prev.microSlot;
//         refreshMicroSlots(capturedDate, capturedSlotTime, gen, latestMicro);
//         return prev; // no state change — just reading prev
//       });
//     }, 20000);

//     return () => clearInterval(poll); // cleanup on date/slot change or unmount
//   }, [form.preferredDate, form.slotTime]);
...
//       try {
//         const generated = generateMicroSlots(form.slotTime).map(s => ({ microSlot: s, booked: false }));
//         const qs2 = `date=${encodeURIComponent(form.preferredDate)}&slotTime=${encodeURIComponent(form.slotTime)}`;
//         const r = await citizenAxios.get(`/availability/micro-slots?${qs2}`);
//         const bookedSet = extractBookedSet(r.data);
...
//       // Fix: axios encodes spaces as "+" in params → "07:00+-+09:00" breaks backend
//       // Use manual query string so spaces become %20
//       // Encode only spaces → %20, leave colons unencoded so backend parses correctly
//       // encodeURIComponent encodes colons too → 10%3A00 → 404. Use manual space encode only.
//       const safeDate     = date.replace(/ /g, "%20");
//       const safeSlotTime = slotTime.replace(/ /g, "%20");
//       const r = await citizenAxios.get(`/availability/micro-slots?date=${safeDate}&slotTime=${safeSlotTime}`);
//       const bookedSet = extractBookedSet(r.data);
...
//     // 3️⃣ Poll every 20 seconds — use captured values, ref for microSlot to avoid stale closure
//     const capturedDate     = form.preferredDate;
//     const capturedSlotTime = form.slotTime;
//     const poll = setInterval(() => {
//       const gen = generateMicroSlots(capturedSlotTime).map(s => ({ microSlot: s, booked: false }));
...
//         const r = await citizenAxios.get(`/availability/micro-slots?date=${safeDate2}&slotTime=${safeSlotTime2}`);
//         const bookedSet = extractBookedSet(r.data);
...
export default function BookAppointment() {
  ...
  // ── Generate + fetch slots when date/slotTime changes ────────────────────
  useEffect(() => {
    // Stop fetching if not on step 1 (Appointment) or no date/slot selected
    ...
    let cancelled = false; // prevent state updates after cleanup
