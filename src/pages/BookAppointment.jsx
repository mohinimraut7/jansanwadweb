...
//   // ── ✅ Username auto-fill + visit count state ─────────────────────────────
//   const [usernameLookupLoading, setUsernameLookupLoading] = useState(false);
//   const [usernameLookupStatus,  setUsernameLookupStatus]  = useState(null); // "found" | "not_found" | null
//   const [visitCount,            setVisitCount]            = useState(null); // ✅ NEW
...
//   // ── ✅ Auto-fill fields + fetch visit count when username is typed (debounced 600ms) ──
//   useEffect(() => {
//     const username = form.username?.trim();

//     if (!username) {
//       setUsernameLookupStatus(null);
//       setVisitCount(null);
//       return;
//     }
...
//         // ── Step 1: Fetch citizen by username ─────────────────────────────
//         const res = await citizenAxios.get(`/citizen/by-username/${username}`);
...
//           // ── Step 2: Fetch appointment count for this citizen ──────────────
//           try {
//             const apptRes = await citizenAxios.get(`/citizen/my-appointments?citizenId=${c._id}`);
//             const count   = apptRes.data?.count ?? apptRes.data?.appointments?.length ?? 0;
//             setVisitCount(count + 1); // +1 → this will be their next visit number
//           } catch {
//             setVisitCount(null);
//           }

//         } else {
//           setUsernameLookupStatus("not_found");
//           setVisitCount(null);
//         }
//       } catch {
//         setUsernameLookupStatus("not_found");
...
//         /* ✅ NEW: Visit count badge */
//         .visit-badge {
//           display: inline-flex;
//           align-items: center;
//           gap: 8px;
//           background: linear-gradient(135deg, #1d6e80, #3a9aaf);
//           color: #fff;
//           padding: 10px 18px;
//           border-radius: 10px;
//           font-size: 13.5px;
...
//           box-shadow: 0 2px 6px rgba(201,150,58,0.5);
//           flex-shrink: 0;
//         }

//         /* ✅ NEW: Username lookup status indicators */
//         .username-found    { border-color: #22c55e !important; box-shadow: 0 0 0 3px rgba(34,197,94,0.15) !important; }
//         .username-notfound { border-color: #ef4444 !important; box-shadow: 0 0 0 3px rgba(239,68,68,0.12) !important; }
//         .username-hint {
//           font-size: 12px;
//           font-weight: 600;
//           margin-top: 6px;
...
//                     {/* ── Username field with auto-fill ── */}
//                     <div className="field">
//                       <label>User Name <span className="req">*</span></label>
//                       <div style={{ position:"relative" }}>
//                         <input
//                           className={`f-input${usernameLookupStatus==="found" ? " username-found" : usernameLookupStatus==="not_found" ? " username-notfound" : ""}`}
//                           value={form.username}
//                           onChange={ch("username")}
//                           placeholder="Username टाका"
//                         />
//                         {/* Spinner while looking up */}
//                         {usernameLookupLoading && (
//                           <span style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", width:16, height:16, border:"2px solid rgba(58,154,175,0.3)", borderTopColor:"var(--teal)", borderRadius:"50%", animation:"spin .7s linear infinite", display:"inline-block" }} />
//                         )}
//                       </div>

//                       {/* ✅ Status hint below username field */}
//                       {!usernameLookupLoading && usernameLookupStatus === "found" && (
//                         <p className="username-hint" style={{ color:"#16a34a" }}>✅ नागरिक सापडला — माहिती auto-fill झाली</p>
//                       )}
//                       {!usernameLookupLoading && usernameLookupStatus === "not_found" && (
//                         <p className="username-hint" style={{ color:"#dc2626" }}>❌ Username सापडला नाही</p>
//                       )}

//                       {/* ✅ NEW: Visit count badge — shown when citizen is found */}
//                       {usernameLookupStatus === "found" && visitCount !== null && (
//                         <div className="visit-badge" style={{ marginTop:10 }}>
...
//                           ["Visited Before", form.visitedBefore?"Yes":"No"],
//                           ["Ward",           form.ward],
//                           // ✅ NEW: show visit number in review too
...
//     numberOfVisitors: "1",
//     visitedBefore:    false,
//     ward:             "",        // ✅ will be auto-filled from last appointment
//     visitorPhoto:     null,
//     photoPreview:     null,
//   });
...
//           // Auto-fill all personal fields from Citizen collection
//           setForm(prev => ({
//             ...prev,
...
//             pincode:      c.pincode      || prev.pincode,
//             address:      c.address      || prev.address,
//             // ✅ Auto-fill ward from last CitizenAppointment via citizenId reference
//             ward:         res.data.lastWard || prev.ward,
//           }));

//           // ✅ Show last visit photo as preview (from CitizenAppointment)
...
//                   {/* ✅ Loading indicator while fetching citizen data */}
//                   {citizenLoading && (
...
//                   {/* ✅ Visit count badge — shown when citizen data is loaded */}
...
//                     </div>
//                     <div className="field">
//                       {/* ✅ Ward auto-filled from last appointment via citizenId */}
//                       <label>Ward <span className="req">*</span></label>
//                       <select className="f-input" value={form.ward} onChange={ch("ward")} style={{ cursor:"pointer" }}>
//                         <option value="">Select Ward</option>
//                         {["Ward A","Ward B","Ward C","Ward D","Ward E","Ward F","Ward G","Ward H","Ward I","Ward J","General"].map(w => <option key={w}>{w}</option>)}
...
//                   {/* ✅ Show last appointment photo as reuse option */}
...
//                           onClick={() => {
//                             // Use last photo URL as-is (backend already has it)
